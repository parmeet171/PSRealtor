...
const RentCard = ({likeCount = 0 ,searchCount = 0  , property}) => {
  ...
  return (
    <>
     ...
     <motion.div  initial = {{opacity : 0 }}  whileInView = {{opacity : 1} } transition  = {{duration : 1 , ease : "easeInOut" ,  }}  className="w-[85%]   mx-auto relative   bg-transparent p-4 flex border-2 items-start gap-4 rounded-xl box-shadow ">
        ...
        <div onClick={() => {navigateTo(`/view/property/${property?._id}`)}}  className="w-[65%] flex flex-col gap-4  cursor-pointer ">
          {/* info */}

          <h1 className="cpaitalize text-xl flex gap-2 font-light">
            {`${property?.bedrooms} BHK ${property?.propertyType} in ${combineAddress()}  `}
          </h1>
          ...
          <div className="flex gap-4  items-center ">
            <div className="flex flex-col gap-1 items-center   font-medium ">
              ...
              <p className="capitalize text-2xl flex items-center gap-1 ">
                ...
                {priceCompressor(property?.rent)}
                <span className="text-sm">/month</span>
              </p>
              {
                property?.securityDeposit && <p className="capitalize text-[1rem]">
            ...
            <div className="flex bg-[#396d8c] p-3 rounded-xl gap-4 items-center ">
              ...
              <div className="flex flex-wrap  gap-4">
                {propertyFeatures.map((feat, index) => {
                  ...
                  return (
                    <p key = {index} className="px-3 py-1 border-2 rounded-full capitalize flex items-center gap-1  ">
                      <span>
                        <IoCheckmark />
                      </span>
                      {feat}
