  ...
  return (
    ...
    {noResults ? <><div className='flex items-center justify-center h-screen  '><h1 className='text-5xl capitalize font-medium '>No Liked property to show </h1></div></> : <div className='w-full  h-auto mt-28 bg-[#001f3f] '>
      ...
      <div className='flex items-center '>
            <h1 className='text-5xl capitalize'> Most Liked  Properties :  </h1>
        </div>
        {
         mlp?.length > 0  && mlp.map((property , index ) => {
            if(property?.property?.propertyFor === "Sell")
            {
              return <SellCard likeCount = {property?.likeCount}  key = {property?.property?._id} property = {property?.property}/>
            }
            else if(property?.property?.propertyFor === "Rent")
            {
              return <RentCard likeCount = {property?.likeCount}   key = {property?.property?._id} property = {property?.property}/>
            }
            else if(property?.property?.propertyFor === "PG")
            {
              return <PgCard likeCount = {property?.likeCount}   key = {property?.property?._id} property = {property?.property}/>
            }
          })
        }
      </div>
