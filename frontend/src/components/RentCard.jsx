...
const RentCard = ({ liked = false, property, myProperties = 0 }) => {
  ...
  const markAsSoldHandler = async (id) => {
    ...
    try {
      ...
      console.log(err);
      toast.error("Something went wrong : while marking as sold");
    }
  };
    ...
    <>
      ...
      >
        ...
        {myProperties == 1 && (
          <div
            onClick={() => {
              markAsSoldHandler(property?._id);
            }}
            className={`${
        ...
        {property.propertyStatus == "Sold" && myProperties == 0 && (
          <div
            className={`${"bg-red-500"}  absolute px-6 py-4 rounded-2xl left-0 top-[-10px]`}
          >
            <button className="capitalize">Sold</button>
          </div>
        )}
        ...
        >
          ...
          <h1 className="cpaitalize text-xl flex gap-2 font-light">
            {property?.bedrooms && <p>{property?.bedrooms} BHK </p>}
            {property?.propertyType && <p>{property?.propertyType}</p>}
            {property.commercialPropertyType && (
              <p>{property.commercialPropertyType}</p>
            )}
          ...
          <div className="flex gap-4  items-center ">
            <div className="flex flex-col gap-1 items-center   font-medium ">
              ...
              <p className="capitalize text-2xl flex items-center gap-1 ">
                <p className="flex items-center gap-1"><FaRupeeSign className="text-[1.1rem]" />
                <span className="text-2xl">{priceCompressor(property?.rent)}</span></p>
                <span className="text-sm self-center">/month</span>
              </p>
            ...
            <div className="flex gap-4">
              ...
              {property?.carpetArea && (
                <div className="font-medium text-xl ">
                  <p className="text-2xl">
                    {property.carpetArea} <span>(sq ft.)</span>
                  </p>
              ...
              {property?.superBuiltUpArea && (
                <div>
                  <p className="text-2xl">
                    {property.superBuiltUpArea} <span>(sq ft.)</span>
                  </p>
              ...
              {property?.builtUpArea && (
                <div>
                  <p className="text-2xl">
                    {property.builtUpArea} <span>(sq ft.)</span>
                  </p>
            ...
            {property?.plotArea && (
              <div>
                <p className="text-2xl">
                  {property.plotArea} <span>(sq ft.)</span>{" "}
                </p>
                <p className="text-xl capitalize">Plot area </p>
              </div>
            )}
            <span className="">|</span>
            ...
            {property?.commercialPropertyType && (
              <div>
                <p className="text-2xl">{property.commercialPropertyType} </p>
          ...
          <div className="flex gap-4 mx-auto bg-[#396d8c] absolute left-10 bottom-0 px-4 py-2 rounded-2xl text-[#f4f5f7]  ">
            ...
            <p></p>
            {/* <p className="capitalize "> {property?.postedBy?.buidler} </p> */}
            <button
              onClick={() => {
                setViewNumber(true);
              }}
        ...
        <div className="absolute capitalize rounded-xl px-4 py-1  right-0  top-[-12px]  bg-[#396d8c] text-[#f4f5f7] ">
          posted on {moment(property?.createdAt).format("DD-MM-YYYY")}
        </div>
